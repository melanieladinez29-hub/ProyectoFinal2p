...
<head>
    ...
    <style>
        ...
        /* Estilo unificado para previsualizaciones */
        .img-mini { width: 60px; height: 60px; object-fit: contain; border-radius: .5rem; background: #f9f9f9; }
    </style>
</head>
<body>
    <header>
    ...
    <a href="index.php?action=admin-categorias">Categorías</a>
    <a href="index.php?action=admin-clientes">Clientes</a>
    <a href="index.php?action=logout">Cerrar sesión</a>
    <a href="index.php?action=admin-proveedores">
    Proveedores
    </a>
    <a href="index.php?action=admin-pedidos">
    Pedidos
</a>
        </div>
    </header>
            ...
            <tbody>
                <?php if (isset($productos) && is_array($productos) && count($productos) > 0): ?>
                    <?php foreach ($productos as $p): ?>
                        <tr>
                            <td><?php echo $p['id']; ?></td>
                            <td><img src="<?php echo htmlspecialchars($p['imagen']); ?>" alt="Cosmético" class="img-mini"></td>
                            <td><strong><?php echo htmlspecialchars($p['nombre']); ?></strong></td>
                            <td>$<?php echo htmlspecialchars($p['precio']); ?></td>
                            <td>
                                <span style="padding: .3rem .8rem; background: #eee; border-radius: .5rem; font-weight: bold;">
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="7" style="padding: 3rem; font-size: 1.8rem; color: #777;">No se encontraron registros en el inventario.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
